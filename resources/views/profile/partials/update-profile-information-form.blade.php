...
<form method="post" action="{{ route('profile.update') }}" >
	...
	<div class="form-group row m-2 @error('currency_id') has-error @enderror">
		<label for="currency" class="col-sm-4 col-form-label col-form-label-sm">Currency : </label>
		<div class="col-sm-8">
			<select name="currency_id" id="currency" value="{{ old('currency_id', $user->name) }}" class="form-control form-control-sm @error('currency_id') is-invalid @enderror" placeholder="Name"></select>
			@error('currency_id')<div class="invalid-feedback fw-lighter">{{ $message }}</div> @enderror
		</div>
	</div>
